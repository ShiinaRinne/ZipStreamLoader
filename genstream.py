...
if __name__ == "__main__":
    ...
    output = input("请输入解压路径, 复制过来就行(拖的会出问题)\r\n")
    for file in os.listdir(output):
        if file == "launcher.exe":
            output = os.path.join(output, "Genshin Impact Game")
            break
    
        
    data = requests.get("https://sdk-static.mihoyo.com/hk4e_cn/mdk/launcher/api/resource?key=eYd89JmJ&launcher_id=18")
    data = data.json()
    genshin_list = data["data"]["game"]["latest"]["segments"]
    genshin_list = [l["path"] for l in genshin_list]
    target_version = data["data"]["game"]["latest"]["version"]
    print("目标版本:", target_version)
    download_and_unzip(genshin_list, output, enable_crc32_check=True)
